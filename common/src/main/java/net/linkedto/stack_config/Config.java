...
public final class Config {
    private static final Path CONFIG_PATH = FabricLoader.getInstance().getConfigDir().resolve("stack_config.json");
        ...
        for (var entry : rawEntries.entrySet()) {
            ...
            if (key.startsWith("#")) {
                ...
                if (tagId != null) {
                    ...
                    for (Holder<Item> holder : registry.getTagOrEmpty(tagKey)) {
                ...
                if (id != null) {
                    registry.get(id).ifPresent(holder -> resolvedSizes.put(holder.value(), size));
                }
            }
        }
    }
